...
#curl -sSL -O http://10.21.21.30/pamc_cloud/easy-rsa.tar.gz
cd "$(dirname "$0")"
tar xzf easy-rsa.tar.gz
cd easy-rsa-master/easyrsa3

# Sadly, openssl is very verbose to std*err* with no option to turn it off.
if ! (sh ./easyrsa --batch init-pki
      # Since the length of CN is limited to 64 bytes, here we cut too long ${cert_ip}
      sh ./easyrsa --batch "--req-cn=$(echo ${cert_ip} | cut -b 1-$(expr 64 - $(echo @$(date +%s) | wc -c)))@$(date +%s)" build-ca nopass
      sh ./easyrsa --batch --subject-alt-name="${sans}" build-server-full master nopass
      sh ./easyrsa --batch build-client-full kubelet nopass
      sh ./easyrsa --batch build-client-full kubecfg nopass) >/dev/null 2>&1; then
...
cat pki/issued/master.crt pki/private/master.key > "${cert_dir}/server.pem"
